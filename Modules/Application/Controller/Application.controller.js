...
const ViewApplication = async (req, res) => {
    try {
        const applications = await Application.findById(id).populate({ path: "createdFor", select: "_id" });
...
const CreateApplication = async (req, res) => {
    try {
        ...
        return res.status(201).json({ massage: "file uploaded successfully", Data: applications });
    } catch (err) {
